...
- The server supports Redis-backed sessions (`SKILLHUB_SESSION_BACKEND=redis`) and Redis-backed distributed rate limiting (`SKILLHUB_RATE_LIMIT_BACKEND=redis`). Production mode requires both to be set to `redis`. An in-memory rate limiter is available only for local/single-instance deployments (`SKILLHUB_RATE_LIMIT_BACKEND=memory`); it is bounded (10000 max buckets, 15min TTL) but not distributed.
- Treat Redis as a production dependency when either Redis-backed sessions or Redis-backed rate limiting is enabled. Startup fails if the configured Redis cannot be reached; runtime Redis failures should be handled as production incidents because they affect session revocation and distributed throttling.
- Login creates a `skillhub_session` HttpOnly cookie when `SKILLHUB_SESSION_BACKEND=redis`. Logout is expected to delete the server-side session and expire the cookie. Bearer token auth remains stateless and does not require Redis.
